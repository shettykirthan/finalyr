...
def ImageGen(text):
    ...
    client_1 = InferenceClient("stabilityai/stable-diffusion-xl-base-1.0", token="")
...
def learnBot():
    ...
    try:
        if image and input_text:
            prompt = f"You are an Ai Agent your job is to correct user's mistakes if it is wrong , correct the user in a friendly manner say your text is wrong and this is the correct way: {input_text}"
            response = model.generate_content([prompt, image])
        elif image:
            prompt = "You are an Ai Agent your job is to correct user's text in the image if it is wrong in a friendly manner say your text is wrong and this is the correct way:"
            response = model.generate_content([prompt, image])
        elif input_text:
            prompt = f"You are an Ai Agent your job is to correct user's text if it is wrong in a friendly manner say your text is wrong and this is the correct way: {input_text}"
